...
pub fn invalid_class_content(selector: felt252) -> ByteArray {
    format!("Invalid ClassHash to upgrade the model `{}`", selector)
}

pub fn invalid_model_schema_upgrade(selector: felt252) -> ByteArray {
    format!("Invalid new schema to upgrade the model `{}`", selector)
}

pub fn invalid_model_layout_upgrade(selector: felt252) -> ByteArray {
    format!("Invalid new layout to upgrade the model `{}`", selector)
}

pub fn invalid_model_version_upgrade(selector: felt252, expected_version: u8) -> ByteArray {
    format!("The new model version of `{}` should be {}", selector, expected_version)
}
